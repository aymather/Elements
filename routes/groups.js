...
router.get('/getgroups', authMiddleware, retreatMiddleware, (req, res) => {
    User.findById(req.user.id)
        .then(user => {
            ...
            return res.json(group_data.reverse());
        })
        .catch(() => {
...
router.post('/addgroup', authMiddleware, retreatMiddleware, (req, res) => {
    ...
    User.findById(req.user.id)
        .then(user => {
            ...
            user.save()
                ...
                .catch(() => {
                    return res.status(500).json({ msg: "Error saving new group"});
                })
        })
        .catch(() => {
            return res.status(500).json({ msg: "Internal server error"});
...
router.post('/removegroup', authMiddleware, retreatMiddleware, (req, res) => {
...
router.post('/editGroupMembers', authMiddleware, retreatMiddleware, (req, res) => {
